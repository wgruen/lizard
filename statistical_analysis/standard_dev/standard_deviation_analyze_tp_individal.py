...
def calculate(data, filename, result, df_summary, cutoff_offset_percent):   
   ...
   # pdf_out = PdfPages('output/multipage.pdf')
    ...
    # each column it its own data set and should be evaluated independently
    ### Get the data mean and std deviation
    for index in range(0, len(Y1[0])):
        Y = Y1[:,index]
        print("index: " + str(index) + "\tlen Y: " + str(len(Y)))
        ...
        result[filename][index]["mean"] = data_mean
        result[filename][index]["std_deviaton"] = data_std
        result[filename][index]["X"] = X
        result[filename][index]["Y"] = Y
        result[filename][index]["title"] = filename + " - " + str(len(Y)) + " data points"
        ...
        result[filename][index]["outliers_Y"] = Y_outliers
        result[filename][index]["outliers_title"] = "Mean " + str(data_mean)\
        ...
        #print("outliers:\n", outliers_data)
        result[filename][index]["outliers_values"] = Y_outliers.copy()
        result[filename][index]["outliers_data"] = outliers_data.copy()
        ...
        #print("outliers:\n", outliers_data)
        result[filename][index]["zeros_data"] = zeros_data.copy()
...
def create_output_pdf(results, df_summary, filebase):    
    ...
    for filename in results:
        for index in results[filename]:
            print("filename: ", filename, "\t index: ", index)
            
            print(str(len(results[filename][index]["X"])) + "\t" +  str(len(results[filename][index]["Y"])))
        
            plt.subplot(1, 2, 1)
            plt.title(filename + " - " + str(len(results[filename][index]["Y"])) + " data points")
            plt.scatter(results[filename][index]["X"], results[filename][index]["Y"], s = 50, c="brown")
        
            plt.subplot(1, 2, 2)
            plt.title("Outliers")
            plt.xlabel(results[filename][index]["outliers_title"])
            plt.scatter(results[filename][index]["X"], results[filename][index]["outliers_Y"], s = 50, c="blue")
            #plt.figure()
            #plot1 = plt.plot()
            ...
            element_outliers = create_output_pdf_for_special_values(results[filename][index]["outliers_data"],\
                         element_outliers, filename)
            
            element_zeros = create_output_pdf_for_special_values(results[filename][index]["zeros_data"],\
